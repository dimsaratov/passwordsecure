...
{
    public Vault EncryptDataToVault(byte[] data, SecureString password)
    {
        try
        {
            byte[] iv = GenerateIv();
            ...
            byte[] key = GetPasswordBytes(password, salt);
            byte[] encryptedData = ExecuteCryptoTransform(
    ...
    public byte[] DecryptDataFromVault(Vault vault, SecureString password)
    {
        try
        {
            byte[] iv = GetIvFromVault(vault);
            byte[] salt = GetSaltFromVault(vault);

            byte[] key = GetPasswordBytes(password, salt);

            byte[] encryptedData = vault.Body;
            byte[] data = ExecuteCryptoTransform(
        ...
        Func<Aes, ICryptoTransform> createCryptoTransform)
    {
        ...
        using ICryptoTransform encryptor = createCryptoTransform(aes);
        ...
        byte[] resultData = dataStream.ToArray();
        return resultData;
    }
